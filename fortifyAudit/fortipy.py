#!/usr/bin/env python
# -*- coding:utf-8 -*-
# https://github.com/nicolasrod/fortipy
# add somethings c4bbage@qq.com
...
class FPR(object):
    ...
    def get_types_overview_of_vulns(self):
        """
        {"Weak Cryptographic Hash": 19, "System Information Leak": 6, "Password Management": 10}
        :return:
            {"Weak Cryptographic Hash": 19, "System Information Leak": 6, "Password Management": 10}
        """
        vulnsOverview = {}
        for t in self.get_types_of_vulns():
            vulnsOverview[t] = self.get_vulns_of_type(t).__len__()
        return vulnsOverview
    ...
    def get_snippet_of_id(self, id):
        return [x for x in self.snippets if x is not None and x.id is not None and x.id.strip() == id.strip()]
